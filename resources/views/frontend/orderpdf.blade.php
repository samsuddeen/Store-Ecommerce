                        ...
                        <table style="float:right;">
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{@$order->area.','.@$order->province.','.@$order->state}}</td>
                                </tr>
                                <tr>
                                    <td style="padding-right:10px;">Email:</td>
                                    <td>{{@$order->email ? @$order->email : @$customer->email}}</td>
                                </tr>
                                <tr>
                                    <td style="padding-right:10px;">Ph No:</td>
                                    <td>{{@$order->phone ? @$order->phone : @$customer->phone}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
            ...
            <table class="table " border='1' width="100%" class="pdf-tables">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td style="text-transform: capitalize;">{{ @$product->product_name }}   
                                @if($product->options)
                                @if($order->user_id=='63')
                            ...
                            <td>{{ @$product->qty }}</td>
                            <td>Rs.{{ formattedNepaliNumber(@$product->price) }}</td>
                            <td>Rs.{{ formattedNepaliNumber(@$product->discount*@$product->qty) }}</td>
                            <td>Rs.{{ formattedNepaliNumber(@$product->sub_total_price-@$product->vatamountfield) }}</td>
                        </tr>
                    @endforeach
                    <tr>
                        <td></td>
                        ...
                        <td></td>
                        <td></td>
                        <td><b>Rs.{{ formattedNepaliNumber($data->sum('sub_total_price')-@$data->sum('vatamountfield')) }}</b></td>
                    </tr>
                    <tr>
                        <td rowspan="4" colspan="4">
                            {{convertNumberIntoWord(@$order->total_price)}}
                        </td>
                        <td >Shipping Charge:</td>
                        <td><strong>Rs.{{formattedNepaliNumber(@$order->shipping_charge)}}</strong></td>
                        
                    </tr>
                    @if(@$order->material_charge)
                    <tr>
                        <td>Material Charge:</td>
                        <td><strong>Rs.{{formattedNepaliNumber(@$order->material_charge)}}</strong></td>
                    </tr>
                    @endif
                    @if($order->coupon_discount_price !=0)
                    <tr>
                        <td>Coupon Discount:</td>
                        <td><strong>Rs.{{formattedNepaliNumber(@$order->coupon_discount_price)}}</strong></td>
                    </tr>
                    @endif
                    {{-- @if($order->vat_amount !=0 && $order->vat_amount >0)
                    <tr>
                        <td>VAT Charge:</td>
                        <td><strong>{{formattedNepaliNumber(@$order->vat_amount)}}</strong></td>
                    </tr>
                    @else
                    <tr>
                        <td>VAT Charge:</td>
                    ...
                    <tr>
                        <td>Grand Total:</td>
                        <td><strong>Rs.{{ formattedNepaliNumber(@$order->total_price) }}</strong></td>
                    </tr>
                </tbody>
            </table>
        @endif
