            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Retailer Offer</h4>
                    </div>
                    <div class="card-body">
                        @if ($topOffer->id)
                            <form action="{{ route('retailer_offer.update', $topOffer->id) }}" method="POST">
                                @method('PATCH')
                            @else
                                <form action="{{ route('retailer_offer.store') }}" method="POST">
                        @endif
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-xl-4 col-md-6 col-12">
                                <div class="mb-1 mt-3">
                                    ...
                                    <input type="checkbox" name="is_fixed" value="1" {{(@$topOffer->is_fixed) ? 'checked' : ''}}>
                                    @error('is_fixed')
                                        <p class="form-control-static text-danger" id="staticInput">{{ $message }}</p>
                                    @enderror
                                </div>
