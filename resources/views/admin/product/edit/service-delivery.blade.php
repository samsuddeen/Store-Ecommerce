<h4> Service and Delivery</h4>
<form action="{{ route('product-edit.serviceAndDelivery', $product->id) }}" method="post">
    @csrf
    @method('PATCH')
    <div class="row">
        <div class="col-md-6">
    ...
    <div class="row">
        ...
        {{-- @dd($product->policy_data) --}}

        <div class="col-md-12 mb-1">
            <ul class="policies-standard"> 
                <li>  
             <input type="radio" name="policy_data" value="0" id="policy" {{(@$product->policy_data==0) ? 'checked' : ''}}> Standard
                </li>
                <li>
            <input type="radio" name="policy_data" value="1" id="policy" {{(@$product->policy_data==1) ? 'checked' : ''}}> Return Policy
                </li>    
            </ul>
        </div>
    ...
    <div class="form-group mb-1">
        <label for="package_weight">Packaged Weight (In number (Kg))</label>
        <input type="number" name="package_weight" class="form-control form-control-sm"
            value="{{ $product->package_weight }}" required>
    </div>
    <div class="form-group package-dimensions">
        <label for="package_dimension">Package Dimension</label>
        <input type="text" class="" name="dimension_length" placeholder="Lenght (Inch)"
            value="{{ $product->dimension_length }}">
        <input type="text" class="" name="dimension_width" placeholder="Width (Inch)"
            value="{{ $product->dimension_width }}">
        <input type="text" class="" name="dimension_height" placeholder="Height (Inch)"
            value="{{ $product->dimension_height }}">
    </div>
    <div class="form-group dangerous-goodspack">
