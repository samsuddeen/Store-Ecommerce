@extends('layouts.app')
@section('title', env('DEFAULT_TITLE') . ' | ' . 'Backup Period')   
@section('content')
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        @if ($backupPeriod->id)
                            <form action="{{ route('backup-period.update', $backupPeriod->id) }}" method="POST">
                                @method('PATCH')
                            @else
                                <form action="{{ route('backup-period.store') }}" method="POST">
                        @endif
                        @csrf
                        <div class="row">
                                ...
                                <div class="mb-1">
                                    ...
                                    <select name="period" id="period" class="form-control form-control-sm" required>
                                        ...
                                        @foreach($periods as $index=>$period)
                                            <option value="{{$period}}" {{(old('period') == $period || @$backupPeriod->period == $period) ? 'selected' : '' }}>{{$index}}</option>
                                        @endforeach
                                    </select>
                                    @error('period')
                            ...
                            <div class="col-xl-4 col-md-6 col-12">
                                <div class="mb-1 mt-2">
                                    ...
                                    <input type="checkbox" name="is_default" value="1" {{(old('is_default') || (@$backupPeriod->is_default == 1)) ? 'checked' : ''}}>
                                    @error('is_default')
                                        <p class="form-control-static text-danger" id="staticInput">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <x-dashboard.button :create="isset($backupPeriod->id)"></x-dashboard.button>
                        </form>
                    </div>
                </div>
            </div>
