    ...
    <div class="ibox invoice" id="printpage">
        ...
        <table class="table table-striped no-margin table-invoice">
            ...
            <tbody>
                ...
                @foreach($sellerOrder->sellerProductOrder as $key => $row)
               
                <tr>
                    <td>{{ ++$key }}</td>
                    <td>
                        @if($row->image !== null )
                            <img src="{{$row->image}}" alt="image not found" class="img-fluid">
                        @elseif($row->product->featured_image() !== null)
                            <img src="{{$row->product->featured_image()}}" alt="image not found" class="img-fluid">
                        @endif

                        @php
                            $options = getOptions($sellerOrder->order->id ?? 0, $ro->product_id ?? 0);
                        @endphp
        ...
        <table class="table no-border">
            ...
            <tbody>
                <tr class="text-right">
                    <td>Subtotal:</td>
                    <td>{{'$ '. $sellerOrder->total + $sellerOrder->total_discount + $sellerOrder->shipping_charge + $sellerOrder->coupon_discount_price  }}</td>
                </tr>

                @if($sellerOrder->total_discount > 0)
                <tr class="text-right">
                    <td>Discount:</td>
                    <td>{{' - $ '.$sellerOrder->total_discount  }}</td>
                </tr>
                @endif

                @if($sellerOrder->coupon_discount_price > 0)
                <tr class="text-right">
                    <td>Coupon Discount:</td>
                    <td>{{'- $ '.$sellerOrder->coupon_discount_price  }}</td>
                </tr>
                @endif
                @if($sellerOrder->shipping_charge > 0)
                <tr class="text-right">
                    <td>Coupon Discount:</td>
                    <td>{{'+ $ '.$sellerOrder->shipping_charge  }}</td>
                </tr>
                @endif
                <tr class="text-right">
                    <td class="font-bold font-18">TOTAL:</td>
                    <td class="font-bold font-18">{{ '$. '.$sellerOrder->total }}</td>
                </tr>
            </tbody>
        </table>
