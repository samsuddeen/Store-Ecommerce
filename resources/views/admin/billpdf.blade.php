                ...
                <tbody>
                    ...
                    {{-- @dd($product) --}}
                        <tr>
                            ...
                            <td>{{ formattedNepaliNumber(@$product->sub_total_price-@$product->vatamountfield) }}</td>
                        </tr>
                    @endforeach
                    <tr>
                        <td></td>
                        <td><b>Total</b></td>
                        {{-- <td>{{ $data->sum('qty') }}</td>
                        <td>{{ $data->sum('price')+$data->sum('discount') }}</td>
                        <td>{{ $data->sum('discount') }}</td> --}}
                        <td></td>
                        <td></td>
                        <td></td>
                        <td><b>{{ formattedNepaliNumber($data->sum('sub_total_price')-@$data->sum('vatamountfield')) }}</b></td>
                    </tr>
                    <tr>
                        <td rowspan="4" colspan="4">
                            {{convertNumberIntoWord($data->sum('sub_total_price')+@$order->shipping_charge)}}
                        </td>
                        <td>Shipping Charge:</td>
                        <td><strong>{{formattedNepaliNumber(@$order->shipping_charge)}}</strong></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td><strong>{{formattedNepaliNumber(@round($order->coupon_discount_price))}}</strong></td>
                    </tr>
                    @endif
                    <tr>
                        <td>Grand Total:</td>
                        <td><strong>{{ formattedNepaliNumber($data->sum('sub_total_price')+@$order->shipping_charge-round($order->coupon_discount_price ?? 0)) }}</strong></td>
                    </tr>
                </tbody>
            </table>
        @endif
