<template>
 <div>
    ...
    <input type="hidden" name="category_id" v-model="selectedCategory" required/>
  ...
  <product-attribute v-if="this.form_show" />

  <product-description v-if="this.form_show" :description="''" :name="'short_description'" :title="'Short Description'" :char_length="'2000'"/>

  <product-long-description v-if="this.form_show" :description="''" :name="'long_description'" :title="'Long Description'" :char_length="'5000'"/>

  <product-price v-if="this.form_show" />
 
  <service-delivery v-if="this.form_show"/>
...
<script>
import ProductAttribute from "./products/ProductAttribute.vue";
import ProductDescription from "./products/ProductDescription.vue";
import ProductLongDescription from "./products/ProductLongDescription.vue";
import ProductPrice from "./products/ProductPrice.vue";
import ServiceDelivery from "./products/ServiceDelivery.vue";
...
export default {
  ...
  components: { Error, ProductAttribute, ProductDescription, ProductPrice, ServiceDelivery, ProductLongDescription },
  created(){
    this.selectedCategory = 62;
  },
  methods: {
    setId() {
      window.CATEGORY_ID = this.selectedCategory;
      this.form_show = true;
    },
    getForm() {
      axios
        .get(BASE_URL + "/forms/" + this.selectedCategory)
        .then((response) => {
