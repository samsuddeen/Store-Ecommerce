...
abstract class PaymentAction
{
    ...
    public function __construct($method = '')
    {
            ...
            headerCheckerManager: new HeaderCheckerManager(
                checkers: [
                    new AlgorithmChecker(
                        supportedAlgorithms: [CredentialData::$JWSAlgorithm],
        ...
        $this->claimCheckerManager = new ClaimCheckerManager(
            checkers: [
                ...
                new AudienceChecker(CredentialData::$AccessToken),
        ...
        $this->jweLoader = new JWELoader(
            ...
            headerCheckerManager: new HeaderCheckerManager(
                checkers: [
                    new AlgorithmChecker(
                        supportedAlgorithms: [CredentialData::$JWEAlgorithm],
                        protectedHeader: true
                    ),
                    new ContentEncryptionAlgorithmChecker(
                        supportedAlgorithms: [CredentialData::$JWEEncrptionAlgorithm],
                        protectedHeader: true
                    )
                ],
                tokenTypes: [
    ...
    protected function EncryptPayload(string $payload, JWK $signingKey, JWK $encryptingKey): string
    {
        ...
        $jws = $this->jwsBuilder
            ...
            ->addSignature($signingKey, [
                "alg" => CredentialData::$JWSAlgorithm,
                "typ" => CredentialData::$TokenType,
            ])
            ->build();
        ...
        $jwe = $this->jweBuilder
            ...
            ->withSharedProtectedHeader([
                "alg" => CredentialData::$JWEAlgorithm,
                "enc" => CredentialData::$JWEEncrptionAlgorithm,
                "kid" => CredentialData::$EncryptionKeyId,
                "typ" => CredentialData::$TokenType,
            ])
     ...
     * @throws Exception
     */
    protected function DecryptToken(string $token, JWK $decryptingKey, JWK $signatureVerificationKey, $api_key=null): string
    {
        $this->claimCheckerManager = new ClaimCheckerManager(
            checkers: [
                new NotBeforeChecker(),
                new ExpirationTimeChecker(),
                new AudienceChecker($api_key),
                new IssuerChecker(["PacoIssuer"]),
            ]
        );
